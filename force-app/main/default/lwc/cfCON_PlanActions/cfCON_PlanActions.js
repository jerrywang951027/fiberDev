import { FlexCardMixin } from "vlocity_cmt/flexCardMixin";
    import {interpolateWithRegex, interpolateKeyValue, fetchCustomLabels } from "vlocity_cmt/flexCardUtility";
          ...
          export default class cfCON_PlanActions extends FlexCardMixin(OmniscriptBaseMixin(LightningElement)){
              @api debug;
              @api recordId;
              @track omniSupportKey = 'cfCON_PlanActions';
              @track record;

              _regexPattern = /\{([a-zA-Z.0-9_]*)\}/g; //for {} fields by default
              @track Label={Billing_Portal_URL:"https://partnerdash.google.com/apps/fibersupport-nightly/billing?"
      };
              connectedCallback() {
              ...
              registerPubSub() {
                pubsub.register('tabLabelChannel', {
                          publish_tab_label: this.handleEventAction.bind(this, data.events[0],0)
                      });
pubsub.register('serviceAccountChannel', {
                          service_account_channel: this.handleEventAction.bind(this, data.events[1],1)
                      });
pubsub.register('reload_plan_card', {
                          data: this.handleEventAction.bind(this, data.events[2],2)
                      });
