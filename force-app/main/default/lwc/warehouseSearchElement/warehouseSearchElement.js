/**
   * @group warehouseSearchElement
   * @description This search element is present on the side bar in warehouse
   * view and it will search the elements entered.
   */
...
import WarehouseMapping
    from "@salesforce/apex/i2msFindSerialAPI.WarehouseMapping";
// Include Find Serial API to get data from server based on search made on screen
import FindSerialRestMethod
    from "@salesforce/apex/i2msFindSerialAPI.FindSerialRestMethod";
// Include Find child Serial API to get child details for Genealogy components
import FindChildSerialRestMethod
    from "@salesforce/apex/i2msFindSerialAPI.FindChildSerialRestMethod";
...
export default class WarehouseSearchElement extends LightningElement {
  ...
  connectedCallback(warehouse, serialnumber, techname, gpn, status) {
    this.loadDataTable(this.warehouse, this.serialnumber,
                       this.techname, this.gpn, this.status)
  }

  // Function to load Main datatable
  @api async loadDataTable(warehouseLocation, serialNumberValue,
                           technameValue, gpnValue, statusValue) {
    ...
    if (warehouseLocation !== '') {
      ...
      warehouseLocation = 
          warehousMappingResponse.find(Arr => Arr.Description__c === warehouseLocation);
    }
    ...
    if (serialNumberValue !== '') {
      var Parameter = ' { "criteria" :{"serialKeys": {"serialNumber": "' +
          serialNumberValue + '"}, "deviceOwnershipHistory" : "REQUIRED", "pageSize" : 2000} }'
    }

    /***
     * If TechName/LDAP was passed
     * -Tech name will be checked in current owner and new owner, 
     *  so there will be 2 API calls for that.
     ...
     */
      ...
      if (gpnValue !== '' && statusValue == '') {
        ...
        Parameter = ' { "criteria" :{ "deviceOwnershipCriteria": { "currentOwners": ' +
            techUsersJSON + ', "gpns": "' + gpnValue + '"}, "deviceOwnershipHistory" : \
            "REQUIRED", "pageSize" : 2000} }'
        ...
        Parameter = ' { "criteria" :{ "deviceOwnershipCriteria": { "newOwners": ' +
            techUsersJSON + ', "gpns": "' + gpnValue + '"}, "deviceOwnershipHistory" : \
            "REQUIRED", "pageSize" : 2000} }'
      }
      ...
      // If TechName/LDAP, Status was passed and GPN was empty
        ...
        // Get data for current owner.
        Parameter = ' { "criteria" :{"deviceOwnershipCriteria": { "currentOwners": ' +
            techUsersJSON + ' , "ownershipStatuses": ["' + statusValue + '"]}, \
            "deviceOwnershipHistory" : "REQUIRED", "pageSize" : 2000} }'
        ApiResponse = await FindSerialRestMethod({
          requestParameter: Parameter
        });
        slotsData = await JSON.parse(ApiResponse);
      ...
      // If TechName/LDAP, Status and GPN was passed 
        ...
        // Get data for current owner.
        Parameter = ' { "criteria" :{"deviceOwnershipCriteria": { "currentOwners": ' +
            techUsersJSON + ' , "gpns": "' + gpnValue + '", "ownershipStatuses": ["' +
            statusValue + '"]}, "deviceOwnershipHistory" : "REQUIRED", "pageSize" : 2000} }'
            ApiResponse = await FindSerialRestMethod({
            requestParameter: Parameter
        });
        slotsData = await JSON.parse(ApiResponse);
      ...
      // If only TechName/LDAP was passed
        ...
        // Get data for current owner.
        Parameter = ' { "criteria" :{"deviceOwnershipCriteria": { "currentOwners": ' +
            techUsersJSON + ' }, "deviceOwnershipHistory" : "REQUIRED", "pageSize" : 2000} }'
        ApiResponse = await FindSerialRestMethod({
          requestParameter: Parameter
        });
        slotsData = await JSON.parse(ApiResponse);
    ...
    else if (gpnValue !== '' && warehouseLocation !== '' && technameValue == ''
        && statusValue == '') {
      Parameter = ' { "criteria" :{"deviceOwnershipCriteria": { "google_location_codes": ["' +
          warehouseLocation.Name + '", "' + warehouseLocation.Name + 'M' + '"], "gpns": "' +
          gpnValue + '"}, "deviceOwnershipHistory" : "REQUIRED", "pageSize" : 2000} }'
    }
    ...
    // If Warehouse and Status was passed, TechName was empty
      ...
      if (statusValue == 'ONHAND') {
        ...
        if (gpnValue !== '') {
          Parameter = ' { "criteria" :{ "devicesSearch": "ON_HAND_DEVICES_ONLY", \
              "on_hand_location_codes" :  ["' + warehouseLocation.Name + '", "' +
              warehouseLocation.Name + 'M' + '"], "serialKeys": {"gpn": "' + gpnValue + '"}, \
              "deviceOwnershipHistory" : "REQUIRED", "pageSize" : 2000} }'
        }
        // OnHand request without GPN, results should be similar to OnHand devices from warehouse view
          ...
          Parameter = ' { "criteria" :{ "devicesSearch": "ON_HAND_DEVICES_ONLY", \
              "on_hand_location_codes" :  ["' + warehouseLocation.Name + '", "' +
              warehouseLocation.Name + 'M' + '"], "deviceOwnershipHistory" : \
              "REQUIRED", "pageSize" : 2000} }'
        }
      } else {
        ...
        if (gpnValue !== '') {
          Parameter = ' { "criteria" :{ "deviceOwnershipCriteria": { "google_location_codes": \
              ["' + warehouseLocation.Name + '", "' + warehouseLocation.Name + 'M' +
              '"], "gpns": "' + gpnValue + '", "ownershipStatuses": ["' + statusValue +
              '"]}, "deviceOwnershipHistory" : "REQUIRED", "pageSize" : 2000} }'
        }
        // Status request without GPN
          ...
          Parameter = ' { "criteria" :{ "deviceOwnershipCriteria": { "google_location_codes": \
              ["' + warehouseLocation.Name + '", "' + warehouseLocation.Name + 'M' + '"], \
              "ownershipStatuses": ["' + statusValue + '"]}, "deviceOwnershipHistory" : \
              "REQUIRED", "pageSize" : 2000} }'
        }
      }
    }
    ...
    if (this.validSelection == true) {
      ...
      if (warehouseLocation !== '' && serialNumberValue == '' && technameValue == '' 
          && (gpnValue !== '' || statusValue !== '')) {
        searchMessage += ' for Location ' + warehouseLocation.Name + ', ' +
            warehouseLocation.Name + 'M';
      }
      ...
      this.searchMessageDisplay = 'Search Result for' + searchMessage + ' (Total ' +
          count + ' Results)';
    }
    this.spinner = false;
  }
  ...
  async callRowAction(event) {
    ...
    // When Genealogy button was selected on screen
      ...
      if (serial_ !== '' || gpn_ !== '') {
        var Parameter = '{"parentSerialKey": {"serial_number": "' + serial_ + '","gpn": "' +
            gpn_ + '"}}'
